...
export default function VoterCard({ className = '', value, show = true, active = false, clickable = false, ...props }: HTMLAttributes<HTMLDivElement> & { className?: string; value?: VotingOption; show?: boolean; active?: boolean; clickable?: boolean;}) {
    return (
        ...
        >
            <div className={'inner ' + ((active ? 'bg-linear-to-br from-sky-500 to-sky-200 text-white dark:from-cyan-600 dark:to-cyan-900 ' : (clickable ? 'hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer ' : '')) + (show ? 'flip' : ''))}>
                <div className='front'>
